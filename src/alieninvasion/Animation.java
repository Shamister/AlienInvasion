...
public class Animation {
	...
	// Amount of time between frames in milliseconds. (How many time in
	// milliseconds will be one frame shown before showing next frame?)
	...
	private void update() {
		if (timeForNextFrame <= System.currentTimeMillis()) {
			...
			if (currentFrameNumber >= numberOfFrames) {
				...
				// If the animation isn't loop then we set that animation isn't
				// active.
				if (!loop)
					active = false;
			}
	...
	public void draw(Graphics2D g2d) {
		...
		if (this.timeOfAnimationCration + this.showDelay <= System
				.currentTimeMillis())
			g2d.drawImage(animImage, x, y, x + frameWidth, y + frameHeight,
